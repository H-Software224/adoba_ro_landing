...
- `getTranslations`/`getLocale`은 이름과 달리 내부적으로 `useSyncExternalStore`를 쓰는
  **훅**이다(로케일 전환 시 리렌더가 안 되는 버그를 막기 위함). 항상 컴포넌트 최상단에서
  조건 없이 호출할 것 — `if (...) return` 뒤에서 호출하면 hooks 규칙 위반.
