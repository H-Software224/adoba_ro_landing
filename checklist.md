...
### 참고 — SSG 불일치, 이후 원인 확인 및 수정 완료 ✅
위에서 발견한 `ƒ`(Dynamic) 문제의 원인 확인 결과, `widgets/footer/ui/Footer.tsx`가 SNS 아이콘 로케일 분기용으로 `getLocale()`(next-intl/server)을 직접 호출하고 있었음 — `getLocale()`은 내부적으로 캐시된 로케일이 없으면 `next/headers`의 `headers()`를 호출하는데, 정적 생성 중 `headers()` 호출은 `DYNAMIC_SERVER_USAGE`를 던져 라우트 전체를 동적 렌더링으로 강제 전환시킴. 프로젝트 전체에서 `getLocale()`을 호출하는 곳은 Footer 한 곳뿐이었고(SSG가 정상이던 `enterprise`의 `Gnb.tsx`는 `getTranslations()`만 사용), `(main)/layout.tsx`가 이 Footer를 홈/기능/요금제/소식/매거진 5개 페이지에 공통으로 쓰다 보니 5곳 전부 영향을 받았음.
- ✅ 수정: `Footer`가 `getLocale()`을 직접 부르는 대신, 부모 `(main)/layout.tsx`가 라우트 `params`에서 이미 알고 있는 `locale`을 prop으로 전달받도록 변경(`Footer({ locale })`). UI/렌더링 결과는 완전히 동일(같은 locale 값), 렌더링 시점만 정적으로 복원됨.
- 재검증: `npm run build` 결과 `/`, `/features`, `/pricing`, `/news`, `/magazine` 전부 `●`(SSG)로 전환 확인(`news`/`magazine`은 `revalidate=3600` ISR 유지). `npm run lint && npm run typecheck` 통과.

---

## 14. 기준 뷰포트 재정립 — PC/모바일 실기기 기준 ✅ 재검증 완료

그동안 "모바일" 검증에 편의상 375×1176을 써왔는데, 이는 실제 기기 값이 아니라 근사치였음. 앞으로는 아래 3개 뷰포트를 공식 기준으로 삼음:

1. **PC**: `1024×1176` — 변경 없음. Tailwind `lg`(1024px) 브레이크포인트와 정확히 일치하므로 계속 유지
2. **모바일 A**: iPhone 12 Pro — `390×844`
3. **모바일 B**: Samsung Galaxy S20 Ultra — `412×915`

### 재검증 결과
Home/Enterprise 페이지를 두 실기기 뷰포트(KO/EN 포함)에서 Header, Hero 스크롤-핀 시퀀스, ProblemSection 캐러셀 위주로 스크린샷 재확인:
- **폭**(390/412)이 기존 검증 폭(375)보다 넓어서, 375px에 맞춰 타이트하게 조정했던 부분(예: 모바일 헤더 CTA 버튼 폰트 11px + `whitespace-nowrap`)도 여유가 더 생겨 한 줄 유지 — 문제 없음, 오히려 여유로워짐
- **높이**(844/915)가 기존 검증 높이(1176)보다 짧지만, Hero의 pin 구간(`min-h-[140dvh]`, `h-dvh`)이 전부 `dvh` 단위라 실제 뷰포트 높이에 비례해서 스케일되므로 구조적으로 깨지지 않음 — Hero 콘텐츠가 다음 섹션에 잘리는 현상 없이 확인됨
- 두 실기기 모두에서 이번 세션에 반영한 기존 수정사항(Header 재구성/토글 스위치, Hero 상단 정렬, FaqSection·ComparisonSection 반응형 간격, ScrollCarousel 등) 전부 정상 작동 확인, **새로 발견된 버그 없음**

### 결론
검증 자체는 코드 수정 불필요했으나(새 버그 없음), 두 실기기 폭을 앞으로 필요할 때 바로 타겟팅할 수 있도록 `tailwind.config.ts`의 `theme.extend.screens`에 커스텀 브레이크포인트로 등록:
```ts
screens: {
  iphone12pro: '390px',
  s20ultra: '412px',
...
```
기존 `sm`/`md`/`lg`/`xl`/`2xl`을 대체하지 않고 추가되는 항목(`extend`)이라 기존 클래스는 전부 그대로 동작. 다른 Tailwind 브레이크포인트와 동일하게 **min-width 기준**이라 `s20ultra:`는 412px "이상" 전체에 적용됨(412px에서만 적용되는 게 아님) — `sm`/`md`/`lg`와 같은 방식. Playwright로 390px에서 `iphone12pro:` 클래스만, 412px에서는 `s20ultra:`(더 큰 min-width라 우선 적용)가 켜지는 것 확인. 앞으로 반응형 작업/검증은 이 3개 뷰포트(1024×1176 PC / `iphone12pro`(390px) / `s20ultra`(412px))를 기준으로 진행하며, 필요 시 `iphone12pro:`/`s20ultra:` 클래스로 실기기 전용 미세조정 가능.
